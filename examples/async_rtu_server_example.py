...
Create an async Modbus RTU server (slave) which can be requested for data or
set with specific values by a client device.

The RTU communication pins can be choosen freely (check MicroPython device/
port specific limitations).
The register definitions of the server as well as its connection settings like
...
from examples.common.rtu_server_common import baudrate, uart_id
...
async def start_rtu_server(slave_addr,
                           ...
                           uart_id,
                           **kwargs):
    """Creates an RTU server (slave) and runs tests"""
