...
"""
Defines the tests for both sync TCP/RTU clients.
"""


def _read_coils_test(client, slave_addr, register_definitions, sleep_fn, **kwargs):
...
def _write_coils_test(client, slave_addr, register_definitions, sleep_fn, **kwargs):
    new_coil_val = 0
...
def _read_hregs_test(client, slave_addr, register_definitions, sleep_fn, **kwargs):
...
def _write_hregs_test(client, slave_addr, register_definitions, sleep_fn, **kwargs):
    new_hreg_val = 44
...
def _write_hregs_beyond_limits_test(client, slave_addr, register_definitions, sleep_fn, **kwargs):
...
def _read_ists_test(client, slave_addr, register_definitions, sleep_fn, **kwargs):
...
def _read_iregs_test(client, slave_addr, register_definitions, sleep_fn, **kwargs):
...
def _reset_registers_test(client, slave_addr, register_definitions, sleep_fn, **kwargs):
...
def run_client_tests(client, slave_addr, register_definitions, exit_on_timeout=False):
    """Runs tests with a Modbus client (client)"""
